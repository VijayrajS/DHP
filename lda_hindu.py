...
article_path = './dataset/hindu'

files = sorted(os.listdir(article_path))
# print(len(files)); exit()
data = {}
data['news_body']  = []
data['time_obj'] = []

if __name__ == '__main__':
    start_date = datetime.strptime(sys.argv[1], '%Y-%m-%d').toordinal()
    end_date = datetime.strptime(sys.argv[2], '%Y-%m-%d').toordinal()
    ...
    for filename in tqdm.tqdm(files):
        ...
        print(date.toordinal(), start_date, end_date)
    ...
    pyLDAvis.save_html(vis, 'lda_hindu.html')
    ...
    ent_list = []
    for e in ents:
        ent_list += [u.text.lower().strip() for u in e]

    from collections import Counter


    print(Counter(ent_list).most_common(30))
