...
def extract_link_article(link):
    ...
    for j in range(len(p_set)):
        if  not article_text.startswith("Also read:") and not article_text.startswith("Listen:") :
            article_text += p_set[j].text + '\n'
    
    with open('dataset/thewire/'+date+'_'+link.split('/')[-1]+'.txt', 'w') as fpw:
        fpw.write(article_text)


with open('results_wire.csv', 'r') as fp:
    pbar = tqdm(total=1)
    ...
    while (line := fp.readline()):
        ...
        file_name = 'dataset/thewire/' + date+'_'+link.split('/')[-1]+'.txt'
